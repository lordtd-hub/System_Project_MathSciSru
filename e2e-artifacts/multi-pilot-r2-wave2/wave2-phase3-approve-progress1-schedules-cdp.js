...
async function approveVisibleWave2Progress1Schedules(client, teacherNo) {
  await qaLogin(client, "teacher", teacherKey(teacherNo));
  let approved = 0;
  let inspected = "";
  for (let attempt = 0; attempt < 20; attempt++) {
    await goto(client, `${baseUrl}/teacher/schedules`);
    const text = await bodyText(client, `teacher ${teacherNo} schedules`);
...
async function main() {
  ...
  try {
    for (let teacherNo = 1; teacherNo <= 11; teacherNo++) {
      result.teachers.push(await approveVisibleWave2Progress1Schedules(client, teacherNo));
    }
    await qaLogin(client, "admin");
    await goto(client, `${baseUrl}/admin/rounds`);
    result.adminRoundExcerpt = (await bodyText(client, "admin rounds after progress1 schedule approvals")).slice(0, 2500);
