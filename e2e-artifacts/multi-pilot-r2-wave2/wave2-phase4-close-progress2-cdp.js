  ...
  try {
    ...
    const closeResult = await evaluate(client, `
      (() => {
        ...
        if (!form) {
          return {
            ...
            forms: forms.map((candidate) => {
              ...
              return {
                roundType: candidate.querySelector('[name="round_type"]')?.value || "",
        ...
        if (!form.checkValidity()) throw new Error("Progress 2 close form invalid before submit");
    ...
    if (closeResult.status !== "submitted") {
      throw new Error("No enabled Progress 2 close form found: " + JSON.stringify(closeResult));
    }
    await sleep(2000);
    await goto(client, `${baseUrl}/admin/rounds`);
