...
async function scoreVisibleWave2Progress2(client, teacherNo) {
  await qaLogin(client, "teacher", teacherKey(teacherNo));
  let submitted = 0;
  let inspected = "";
  for (let attempt = 0; attempt < 20; attempt++) {
    await goto(client, `${baseUrl}/teacher/progress2`);
    const text = await bodyText(client, `teacher ${teacherNo} progress2`);
    ...
    const result = await evaluate(client, `
      (() => {
        const forms = Array.from(document.querySelectorAll("form")).filter((form) => {
          ...
          return text.includes("MULTI-PILOT-R2 Wave 2 Project") && text.includes("Progress 2");
        });
        const form = forms[0];
        ...
        if (comment) {
          comment.value = "Wave 2 Progress 2 score submitted after confirmed schedule and evidence review.";
        ...
        if (!button) throw new Error("Progress 2 score button not enabled");
        if (!form.checkValidity()) {
          ...
          throw new Error("Progress 2 score form invalid: " + invalid.join(","));
        }
        const section = form.closest("section") || form.parentElement;
    ...
    `, `teacher ${teacherNo} submit progress2 score`);
...
async function main() {
  ...
  try {
    for (let teacherNo = 1; teacherNo <= 11; teacherNo++) {
      result.teachers.push(await scoreVisibleWave2Progress2(client, teacherNo));
    }
    await qaLogin(client, "admin");
    await goto(client, `${baseUrl}/admin/rounds`);
    result.adminRoundExcerpt = (await bodyText(client, "admin rounds after progress2 scoring")).slice(0, 2600);
