...
async function approveVisibleWave2Progress2Schedules(client, teacherNo) {
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    ...
    `, `teacher ${teacherNo} approve progress2 schedule`);
...
async function main() {
  ...
  try {
    for (let teacherNo = 1; teacherNo <= 11; teacherNo++) {
      result.teachers.push(await approveVisibleWave2Progress2Schedules(client, teacherNo));
    }
    await qaLogin(client, "admin");
    await goto(client, `${baseUrl}/admin/rounds`);
    result.adminRoundExcerpt = (await bodyText(client, "admin rounds after progress2 schedule approvals")).slice(0, 2500);
