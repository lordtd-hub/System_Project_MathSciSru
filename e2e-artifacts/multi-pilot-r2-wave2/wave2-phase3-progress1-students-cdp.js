...
async function openProgress1Round(client) {
  ...
  const result = await evaluate(client, `
    (() => {
      ...
      const form = forms.find((candidate) => {
        ...
        return roundType?.value === "PROGRESS_1" && button;
      });
  ...
  if (!afterText.includes(expectedOfferingTitle)) throw new Error("Expected offering missing after Progress 1 open action");
  return result;
}
...
async function submitStudentProgress1(client, projectNo) {
  ...
  if (!afterEvidenceText.includes("Wave 2 Progress 1 evidence") && !afterEvidenceText.includes("เธเธฑเธเธ—เธถเธเน€เธญเธเธชเธฒเธฃ")) {
  ...
  if (!afterScheduleText.includes("W2-P1") && !afterScheduleText.includes("เธชเนเธเธเธญเธเธฑเธ”")) {
