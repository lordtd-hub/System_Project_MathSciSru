...
async function scoreVisibleWave2Final(client, teacherNo) {
  await qaLogin(client, "teacher", teacherKey(teacherNo));
  let submitted = 0;
  let inspected = "";
  for (let attempt = 0; attempt < 30; attempt++) {
    await goto(client, `${baseUrl}/teacher/final`);
    const text = await bodyText(client, `teacher ${teacherNo} final`);
    ...
    const result = await evaluate(client, `
      (() => {
        ...
        if (comment) {
          comment.value = "Wave 2 Final presentation score submitted after confirmed schedule and evidence review.";
        ...
        if (!button) throw new Error("Final score button not enabled");
        if (!form.checkValidity()) {
          ...
          throw new Error("Final score form invalid: " + invalid.join(","));
        }
        const section = form.closest("section") || form.parentElement;
    ...
    `, `teacher ${teacherNo} submit final score`);
...
async function main() {
  ...
  try {
    for (let teacherNo = 1; teacherNo <= 11; teacherNo++) {
      result.teachers.push(await scoreVisibleWave2Final(client, teacherNo));
    }
    await qaLogin(client, "admin");
    await goto(client, `${baseUrl}/admin/rounds`);
    result.adminRoundExcerpt = (await bodyText(client, "admin rounds after final scoring")).slice(0, 3200);
