...
async function main() {
  ...
  const result = await client.send("Runtime.evaluate", {
    ...
    expression: `(() => {
      ...
      set('responsibility', '23');
      set('research_process', '23');
      set('problem_solving', '23');
      set('communication', '14');
      set('professionalism', '9');
      set('comment', 'นักศึกษาดำเนินงานต่อเนื่องและปรับแก้รายงานตามข้อเสนอแนะครบถ้วน');
      ...
      const button = [...document.querySelectorAll('button')].find((candidate) => candidate.innerText.includes('บันทึกคะแนนสรุป'));
      if (!button) return { submitted: false, reason: 'missing advisor score button', url: location.href };
