  ...
  return (
    <section className="panel dashboard-console-panel">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
        {metrics.map((metric) => (
          <div key={metric.label} className={`rounded-md border border-line border-l-[3px] p-2.5 ${panelToneClassName[metric.tone]}`}>
            <div className="flex items-center justify-between gap-2">
              <AdminQueueBadge tone={metric.tone}>{metric.label}</AdminQueueBadge>
              <span className="text-lg font-semibold tabular-nums leading-none">{metric.count}</span>
            </div>
            {metric.description ? <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-muted">{metric.description}</p> : null}
          </div>
        ))}
      </div>
    </section>
...
export function AdminQueueSection({
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="admin-queue-scroll" data-scrollable={count > 5 ? "true" : undefined}>
