...
import { submitProgress2Score } from "../scoringActions";
          ...
          return (
            <section key={project.id} id={`project-${project.id}`} className="panel teacher-review-card scroll-mt-24">
              ...
              <RecoverableScoreActionForm
                action={submitProgress2Score}
                storageKey={`teacher-progress-2-score-recovery:${session.user.id}:${project.id}`}
              ...
              >
                ...
                <SubmitButton disabled={!rubric?.items.length} pendingText="กำลังส่งคะแนน..." confirmMessage={previous ? "ยืนยันส่งคะแนน Progress 2 ที่แก้ไขหรือไม่? ระบบจะเก็บรายการแก้ไขเป็นหลักฐาน" : "ยืนยันส่งคะแนนการสอบความก้าวหน้าครั้งที่ 2 หรือไม่?"} scoreGuard autoRecovery={false}>
                  {previous ? "ยืนยันส่งคะแนนที่แก้ไข" : "ยืนยันส่งคะแนนการสอบความก้าวหน้าครั้งที่ 2"}
                </SubmitButton>
              </RecoverableScoreActionForm>
            </section>
          );
