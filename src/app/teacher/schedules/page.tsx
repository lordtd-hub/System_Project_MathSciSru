...
export default async function TeacherSchedulesPage({
  ...
  const scheduleWorkloadMetrics = [
    { label: "ต้องดำเนินการ", count: pendingReviewSchedules.length, tone: "action" as const, description: "คำขอวันสอบที่รอท่านอนุมัติ" },
    { label: "รอ", count: waitingSchedules.length, tone: "waiting" as const, description: "รายการที่รอกรรมการท่านอื่นหรือรอสรุปสถานะ" },
    { label: "เสร็จแล้ว", count: confirmedScheduleCalendar.length, tone: "completed" as const, description: "ตารางสอบที่ยืนยันแล้ว" },
    { label: "ส่งกลับ", count: returnedSchedules.length, tone: "returned" as const, description: "คำขอที่มีผู้ไม่สะดวก" },
    { label: "ยังไม่เปิด", count: 0, tone: "locked" as const, description: "รอบที่ปิดแล้วไม่ใช่งานอนุมัติ" }
  ];
  const uiMode = await getUiMode();

  if (uiMode === "figma") {
    return (
      <div className="figma-dashboard-page figma-teacher-schedules">
        <FigmaPageHeader
          eyebrow="ตารางสอบ"
          title="ตารางสอบที่เกี่ยวข้อง"
          description="แยกคำขอที่ต้องอนุมัติออกจากรายการที่รอกรรมการท่านอื่น รายการส่งกลับ และตารางที่ยืนยันแล้ว เพื่อให้ตรวจงานจำนวนมากได้เร็วขึ้น"
        />
        <ActionFeedback success={params?.success} error={params?.error} />
        <div className="figma-kpi-grid">
          {scheduleWorkloadMetrics.map((metric) => (
            <FigmaMetricCard
              key={metric.label}
              label={metric.label}
              value={metric.count}
              description={metric.description}
              tone={metric.tone === "action" ? "action" : metric.tone === "waiting" ? "warning" : metric.tone === "completed" ? "success" : metric.tone === "returned" ? "danger" : "muted"}
            />
          ))}
        </div>
        ...
        <div className="figma-dashboard-grid">
          <FigmaPanel
            ...
            description="รายการที่ยังรอการตัดสินใจจากท่าน จัดไว้ก่อนรายละเอียดเพื่อให้สแกนได้เร็ว"
            tone="action"
          >
            {pendingReviewSchedules.length ? (
              <div className="space-y-2">
                {pendingReviewSchedules.map((schedule) => (
                  <a key={`${schedule.id}-figma-queue`} className="figma-action-row" data-tone="action" href={`#schedule-${schedule.id}`}>
                    <div>
                      <strong>{schedule.project.currentTitleTh ?? "ยังไม่มีชื่อหัวข้อ"}</strong>
                      <p>
                        {schedule.project.student.studentCode} {schedule.project.student.firstNameTh} {schedule.project.student.lastNameTh}
                      </p>
                      <small>
                        {scheduleRoundLabel(schedule.roundType ?? schedule.assessmentKind)} · {formatThaiScheduleRange(schedule.proposedStartAt, schedule.proposedEndAt)}
                        {schedule.room ? ` · ห้อง ${schedule.room}` : ""}
                      </small>
                    </div>
                    <FigmaStatusBadge tone="action">ต้องดำเนินการ</FigmaStatusBadge>
                  </a>
                ))}
              </div>
            ) : (
              <EmptyState title="ยังไม่มีรายการรออนุมัติวันสอบ" description="รายการที่อนุมัติแล้วหรือรอบที่ปิดแล้วจะไม่แสดงเป็นงานที่ต้องดำเนินการ" />
            )}
          </FigmaPanel>

          <FigmaPanel
            title="สถานะอื่น"
            description="รายการที่รอคนอื่น ดำเนินการเสร็จแล้ว หรือถูกส่งกลับ แยกจากคิวหลัก"
            tone="muted"
          >
            <div className="space-y-2">
              {waitingSchedules.slice(0, 8).map((schedule) => (
                <div key={`${schedule.id}-figma-waiting`} className="figma-action-row" data-tone="warning">
                  <div>
                    <strong>{schedule.project.currentTitleTh ?? "ยังไม่มีชื่อหัวข้อ"}</strong>
                    <p>
                      {schedule.project.student.studentCode} {schedule.project.student.firstNameTh} {schedule.project.student.lastNameTh}
                  ...
                  <FigmaStatusBadge tone="warning">{scheduleStatusLabel(schedule.status)}</FigmaStatusBadge>
                </div>
              ))}
              {returnedSchedules.slice(0, 8).map((schedule) => (
                <div key={`${schedule.id}-figma-returned`} className="figma-action-row" data-tone="muted">
                  <div>
                    <strong>{schedule.project.currentTitleTh ?? "ยังไม่มีชื่อหัวข้อ"}</strong>
                    <p>
                      {schedule.project.student.studentCode} {schedule.project.student.firstNameTh} {schedule.project.student.lastNameTh}
                  ...
                  <FigmaStatusBadge tone="danger">{scheduleStatusLabel(schedule.status)}</FigmaStatusBadge>
                </div>
              ))}
              {!waitingSchedules.length && !returnedSchedules.length ? (
                <p className="text-sm text-muted">ยังไม่มีรายการรอหรือส่งกลับในขณะนี้</p>
              ) : null}
            </div>
          </FigmaPanel>
        </div>
        ...
        <FigmaPanel
          title="รายละเอียดคำขอที่ต้องพิจารณา"
          description="ฟอร์มอนุมัติและไม่อนุมัติเป็น action เดิมทั้งหมด เพียงจัด layout ให้อ่านบริบทกับการตัดสินใจแยกกัน"
          tone="action"
        >
          <div className="space-y-4">
            {pendingReviewSchedules.length ? pendingReviewSchedules.map((schedule) => {
              ...
              return (
                <section key={`${schedule.id}-figma-detail`} id={`schedule-${schedule.id}`} className="figma-panel scroll-mt-24" data-tone="action">
                  <header>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <h2>{scheduleRoundLabel(schedule.roundType ?? schedule.assessmentKind)}</h2>
                        <p>
                          {schedule.project.student.studentCode} {schedule.project.student.firstNameTh} {schedule.project.student.lastNameTh}
                        </p>
                      </div>
                  ...
                  <FigmaReviewLayout
                    context={(
                      <div className="space-y-3">
                        <div>
                          <div className="text-sm font-semibold">{schedule.project.currentTitleTh ?? "ยังไม่มีชื่อหัวข้อ"}</div>
                        ...
                        {schedule.note ? <MarkdownLatexViewer value={schedule.note} /> : null}
                        <div className="text-sm">
                          <div className="font-semibold">กรรมการ</div>
                          <div className="mt-2 flex flex-wrap gap-2">
                            {schedule.project.committeeAssignments.map((assignment) => (
                              <span key={`${schedule.id}-figma-${assignment.id}`} className="rounded-full border border-line px-3 py-1 text-xs">
                    ...
                    action={canReviewSchedule ? (
                      <div className="space-y-3">
                        <div className="text-sm font-semibold">พิจารณาวันสอบ</div>
                        <form action={reviewExamSchedule}>
                        ...
                        <form action={reviewExamSchedule} className="space-y-2">
                          ...
                          <label htmlFor={`figma-schedule-comment-${schedule.id}`}>เหตุผลกรณีไม่อนุมัติ / เวลาที่สะดวกกว่า</label>
                          <textarea id={`figma-schedule-comment-${schedule.id}`} name="comment" rows={4} required placeholder="กรุณาระบุเหตุผลเพื่อให้นักศึกษาเสนอวันสอบใหม่ได้ถูกต้อง" />
                          <SubmitButton className="button-danger" pendingText="กำลังบันทึก...">ไม่อนุมัติ / ขอเปลี่ยนเวลา</SubmitButton>
                        </form>
                      </div>
                    ) : (
                      <div className="rounded-md border border-line bg-paperSoft p-3 text-sm text-muted">
                        {schedule.status !== "PROPOSED"
        ...
        <FigmaPanel
          ...
          description="รายการอ่านอย่างเดียวสำหรับวางแผนเข้าร่วมหรือหลีกเลี่ยงเวลาซ้อนกัน"
          tone="success"
        >
          <div className="space-y-2">
            {confirmedScheduleCalendar.length ? confirmedScheduleCalendar.map((schedule) => (
              <div key={`${schedule.id}-figma-confirmed`} className="figma-schedule-row">
                <div>
                  <strong>{scheduleRoundLabel(schedule.roundType ?? schedule.assessmentKind)}</strong>
                  <p>
                    {schedule.project.student.studentCode} {schedule.project.student.firstNameTh} {schedule.project.student.lastNameTh}
