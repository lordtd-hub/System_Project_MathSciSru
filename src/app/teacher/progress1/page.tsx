...
export default async function TeacherProgress1Page({
    ...
    return (
      <section key={`${project.id}-figma`} id={`project-${project.id}`} className="scroll-mt-24 rounded-lg border border-line bg-surface p-4 shadow-sm">
        <FigmaReviewLayout
            ...
            <div className="space-y-4">
              <div>
                <div className="flex flex-wrap gap-2">
                  ...
                  <FigmaStatusBadge tone="muted">Progress 1</FigmaStatusBadge>
              ...
              <div className="rounded-lg border border-line bg-paperSoft p-3 text-sm">
                <div className="font-semibold text-ink">เอกสาร/หลักฐานที่นักศึกษาส่งสำหรับการสอบความก้าวหน้าครั้งที่ 1</div>
                {evidenceSubmission ? (
          ...
          action={
            <form action={submitProgress1Score} className="space-y-4 rounded-lg border border-line bg-paperSoft p-4">
              <input type="hidden" name="project_id" value={project.id} />
              <div>
                <h3 className="text-sm font-semibold text-ink">บันทึกคะแนน Progress 1</h3>
  ...
  if (uiMode === "figma") {
    return (
      <div className="figma-dashboard-page figma-teacher-progress1">
        <FigmaPageHeader
          eyebrow="Progress 1"
          title="บันทึกคะแนนการสอบความก้าวหน้าครั้งที่ 1"
          description="จัดคิวงานให้คะแนนไว้ก่อนรายละเอียดหลักฐาน โดยยังใช้ฟอร์มและขั้นตอนเดิมทั้งหมด"
        />

        <div className="figma-kpi-grid">
          <FigmaMetricCard label="ต้องดำเนินการ" value={projects.length} description="พร้อมให้คะแนน Progress 1" tone="action" />
        ...
        <FigmaPanel
          title="คิว Progress 1 ที่ต้องให้คะแนน"
        ...
        >
          {projects.length ? (
            ...
            <EmptyState title="ยังไม่มีโครงการการสอบความก้าวหน้าครั้งที่ 1 ที่ต้องให้คะแนน" description="รายการจะแสดงเมื่อท่านเป็นประธานกรรมการหรือกรรมการ และกรรมการยืนยันวันสอบความก้าวหน้าครั้งที่ 1 ครบแล้ว" />
          )}
        </FigmaPanel>
        ...
        >
          <div className="space-y-4">
            {projects.length ? projects.map(renderFigmaProject) : (
              <EmptyState title="ยังไม่มีรายการให้ประเมินในรอบนี้" description="คิวจะว่างเมื่อไม่มีโครงการที่พร้อมให้คะแนน Progress 1 สำหรับอาจารย์คนนี้" />
            )}
          </div>
        </FigmaPanel>
      </div>
